...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method === "POST") {
    try {
      const { prompt, context } = req.body as { prompt: string; context?: string }; // Aggiunto context

      if (!prompt) {
        return res.status(400).json("Missing body.");
      }

      const fullPrompt = context ? `${context} ${prompt}` : prompt; // Usa il contesto se presente
